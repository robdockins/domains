...
Definition is_mub_complete (A:preord) :=
  forall (M:finset A) (x:A), upper_bound x M ->
        exists mub:A, minimal_upper_bound mub M /\ mub ≤ x.

Definition mub_closed (A:preord) (X:finset A) :=
  forall M:finset A, M ⊆ X -> forall x:A, minimal_upper_bound x M -> x ∈ X.

Definition minimum (A:preord) (x:A) :=
  forall y, y ≤ x -> x ≤ y.

Record plotkin_order (A:preord) :=
  PlotkinOrder
  { mub_complete : is_mub_complete A
  ; mub_closure : finset A -> finset A
  ; mub_clos_incl : forall M:finset A, M ⊆ mub_closure M
  ; mub_clos_mub : forall (M:finset A), mub_closed A (mub_closure M)
  ; mub_clos_smallest : forall (M X:finset A),
        M ⊆ X ->
        mub_closed A X -> 
        mub_closure M ⊆ X
  }.
Arguments mub_closure [A] p _.

Lemma mub_clos_mono : forall A (H:plotkin_order A),
  forall (M N:finset A),
...
Lemma mub_clos_idem : forall A (H:plotkin_order A), 
  forall (M:finset A),
...
Section dec_lemmas.
  ...
  Lemma mub_finset_dec : forall (M:finset A) (x:A),
  ...
  Proof.
    ...
    split; auto.
    intros.
    destruct (mub_complete A Hplt M b) as [b0 [??]]; auto.
...
Definition directed {T:set.theory} {A:preord} (X:set T A) :=
  forall M:finset A, M ⊆ X -> exists x, upper_bound x M /\ x ∈ X.
...
Section normal_sets.
  ...
  Definition normal_set (X:finset A) :=
    forall z, directed (finsubset A (fun x => x ≤ z) (fun x => eff_ord_dec A Heff x z) X).

  Definition has_normals :=
    forall X:finset A, { Z:finset A | X ⊆ Z /\ normal_set Z }.
    
  Section plt_normal.
    Hypothesis Hplt : plotkin_order A.
    ...
    Proof.
      ...
      red; intros.
      red; simpl; intros.
      destruct (mub_complete A Hplt M z).
      red; intros.
      apply H in H0.
      ...
      split; auto.
      apply (mub_clos_mub A Hplt X M); auto.
      red; intros.
      apply H in H2.
  ...
  Section normal_mubs.
    ...
    Let Y := proj1_sig (normal_has_ubs Q H X H0).
    Let H1 := proj1 (proj2_sig (normal_has_ubs Q H X H0)).
    Let H2 := proj2 (proj2_sig (normal_has_ubs Q H X H0)).
  ...
  Lemma normal_sub_mub_closed_dec Q : normal_set Q ->
    forall (M:finset A), M ⊆ Q -> { mub_closed A M }+{ ~mub_closed A M }.
  Proof.
    intros HQ M HM.
    unfold mub_closed.
    set (P' (N:finset A) := N ⊆ M -> forall x, minimal_upper_bound x N -> x ∈ M).
    ...
    destruct (normal_has_mubs Q HQ x) as [MUBS [?[??]]].
  ...
  Lemma normal_set_mub_closed_sets Q : normal_set Q ->
    { CLS : finset (finset A) | 
      forall X, X ∈ CLS <-> (X ⊆ Q /\ mub_closed A X) }.
  Proof.        
    intros.
    ...
    assert { XS:finset (finset A) | XS ⊆ SUBS /\ 
      forall X, X ∈ XS <-> (X ∈ SUBS /\ mub_closed A X) }.
    revert H0.
    generalize SUBS.
    clear SUBS.
    ...
    destruct (normal_sub_mub_closed_dec Q H a).
  ...
  Lemma mub_closed_intersect : forall (X Y:finset A),
    mub_closed A X -> mub_closed A Y ->
    mub_closed A (fin_intersect A OD X Y).
  Proof.
    repeat intro.
    ...
    red; intros.
    apply H1 in H3.
    apply fin_intersect_elem in H3.
    destruct H3; auto.
    apply (H0 M); auto.
    red; intros.
    apply H1 in H3.
  ...
  Lemma normal_set_mub_closed Q : normal_set Q -> mub_closed A Q.
  Proof.
    repeat intro.
    destruct (normal_has_mubs Q H M H0) as [MUBS [?[??]]].
    destruct (H4 x) as [m [?[??]]].
    destruct H1; auto.
    apply H2.
    apply member_eq with m; auto.
    split; auto.
    destruct H1. apply H8; auto.
    destruct H7; auto.
  Qed.

  Lemma normal_set_mub_closure Q : normal_set Q ->
    forall (M:finset A), M ⊆ Q ->
      { CL:finset A | M ⊆ CL /\ mub_closed A CL /\
          forall CL':finset A, M ⊆ CL' -> mub_closed A CL' -> CL ⊆ CL' }.
  Proof.
    intros.
    destruct (normal_set_mub_closed_sets Q H) as [CLS ?].
    ...
    intros. rewrite <- H3; auto.
    split.
    cut (forall x, x ∈ CLS' -> mub_closed A x).
  ...
  Lemma mub_closed_normal_set : forall Q,
    is_mub_complete A ->
    mub_closed A Q -> normal_set Q.
  Proof.
    repeat intro.
  ...
  Definition norm_closure X := 
    match Hnorm X with
    | exist Q (conj HQ1 HQ2) => proj1_sig (normal_set_mub_closure Q HQ2 X HQ1)
    end.
    
  Program Definition norm_plt : plotkin_order A :=
    PlotkinOrder A _ norm_closure _ _ _.
  Next Obligation.
    red; intros.
    destruct (Hnorm M) as [Q [??]].
    destruct (normal_has_mubs Q H1 M H0) as [MUBS [?[??]]].
    destruct (H4 x) as [m [?[??]]]; auto.
    exists m; split; auto.
  Qed.
  Next Obligation.
    repeat intro.
    unfold norm_closure.
    destruct (Hnorm M) as [Q [??]].
    destruct (normal_set_mub_closure Q n M i).
    simpl.
    destruct a0.
    apply H0. auto.
  Qed.    
  ...
  Next Obligation.
    ...
    destruct (normal_set_mub_closure Q n M i).
    simpl in *.
    destruct a0. destruct H3.
    apply H4; auto.
  Qed.    

End normal_sets.

Lemma prod_has_normals (A B:preord)
  (HAeff:effective_order A)
  (HBeff:effective_order B)
  (HA:plotkin_order A)
  (HB:plotkin_order B) :
  has_normals (A×B) (effective_prod HAeff HBeff).
Proof.
  red; intros.
  ...
  repeat intro.
  destruct x as [a b].
  destruct (mub_complete A HA (image π₁ M) a).
  red; intros.
  ...
  rewrite H1.
  destruct H0; auto.
  destruct (mub_complete B HB (image π₂ M) b).
  red; intros.
  ...
  apply (mub_clos_mub A HA (image π₁ X) (image π₁ M)).
  red; intros.
  ...
  apply (mub_clos_mub B HB (image π₂ X) (image π₂ M)).
  red; intros.
  apply image_axiom2 in H1. destruct H1 as [y [??]].
  apply H in H1.
  destruct y.
...
Definition plotkin_prod (A B:preord)
  (HAeff:effective_order A) (HBeff:effective_order B)
  (HA:plotkin_order A) (HB:plotkin_order B)
  : plotkin_order (A×B)
  := norm_plt (A×B)
         (effective_prod HAeff HBeff)
         (prod_has_normals A B HAeff HBeff HA HB).
